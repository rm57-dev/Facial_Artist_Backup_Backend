  ...
  async login(req, res) {
    ...
    try {
      ...
      if (usuarios.length === 0) {
        return res.status(401).json({ error: "Uusario no encontrado" });
      }
      ...
      // 🔹 LEFT JOIN para que funcione incluso si el rol no tiene permisos
      ...
      const token = jwt.sign({ id: usuario.id_usuario, rol: usuario.id_rol }, "secreto_super_seguro", {
        expiresIn: "2h",
      });
